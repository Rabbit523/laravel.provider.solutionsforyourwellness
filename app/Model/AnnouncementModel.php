...
class AnnouncementModel extends Authenticatable
{
  ...
  */
  public static function GetLatest($user_id,$user_city){
    $announce = AnnouncementModel::orWhereNull('visible_providers')
								->orWhereNull('visible_cities')
								->orWhereNull('visible_cities','=','')
								->orWhereNull('visible_providers','=','')
								->orWhereRaw('FIND_IN_SET('.$user_id.',visible_providers)')
								->orWhereRaw('FIND_IN_SET("'.$user_city.'",visible_cities)')
								->orderBy('id', 'desc')
                ->get()->toArray();
    if(empty($announce)){
      return false;
