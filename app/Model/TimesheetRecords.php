...
class TimesheetRecords extends Authenticatable
{
    use Notifiable;
	protected $table 		= 	'providers_timesheet';
  ...
  */
  public static function GetData($search="",$start,$length,$column_id,$column_order){
    ...
    if($search){
		...
		if($column_order){
			$usersdata = ClockInAway::selectRaw('clock_in_away.*,users.first_name,users.last_name,clinics.name')->join('clinics','clinics.id','=','clock_in_away.clinic_id')->join('users','users.id','=','clock_in_away.user_id')->orderBy($column_name[$column_id],$column_order)->limit($length)->offset($start)->get();
		}else{
			$usersdata = ClockInAway::selectRaw('clock_in_away.*,users.first_name,users.last_name,clinics.name')->join('clinics','clinics.Id','=','clock_in_away.clinic_id')->orderBy('id','desc')->limit($length)->offset($start)->get();
		}   
    }
        if(empty($usersdata)){
          return false;
